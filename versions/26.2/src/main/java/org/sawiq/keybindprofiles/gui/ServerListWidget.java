...
final class ServerListWidget {
    ...
    private void addRow(RefreshRequest request, String server, int rowX, int rowY) {
        Button serverButton = Button.builder(Component.literal(server), button -> {
            request.serverInputField().setValue(server);
        }).bounds(rowX, rowY, 196, KeyBindProfileScreenLayout.SERVER_ROW_HEIGHT).build();

        Button removeButton = Button.builder(Component.translatable("keybindprofiles.remove_server"), button -> {
            request.removeServer().accept(server);
        }).bounds(rowX + 204, rowY, 96, KeyBindProfileScreenLayout.SERVER_ROW_HEIGHT).build();
    ...
    private record ServerButtonPair(Button serverButton, Button removeButton) {
    }
